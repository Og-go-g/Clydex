...
// Falls back to in-memory token-bucket if UPSTASH env vars are
// not set, so the app works in local development without Redis.
...
const TIERS: Record<string, Tier> = {
  auth: { maxTokens: 5, refillRate: 5 / 60 }, // 5/min
  expensive: { maxTokens: 10, refillRate: 10 / 60 }, // 10/min
  default: { maxTokens: 30, refillRate: 30 / 60 }, // 30/min
};
...
export async function middleware(request: NextRequest) {
  ...
  if (upstashLimiters) {
    // Production path: distributed sliding-window via Upstash Redis
    ...
    // Dev fallback: in-memory token-bucket
    maybeCleanup();
